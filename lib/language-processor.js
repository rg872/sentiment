...
module.exports = {
    ...
    getCustomTokens: function(tokens, languageCode) {
        var language = this.getLanguage(languageCode);
         // return default token if none was specified 
        if(!(language.customTokenization && language.customLabels)) {
            return tokens;
        } else {
            return language.customTokenization(tokens, language.customLabels);
        }  
    }
};
